...
- Powers: 23 of 25 player-card-relevant STS1 powers have native implementations and acceptance, plus the native Melodia support model. `DazzlingDownPower` and `FreshlySqueezedCucumberPower` are used by potions and are deferred with those potions to Phase N6.
